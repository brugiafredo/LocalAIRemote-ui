...
function capabilityMarkers(model: ModelInfo): string {
  const markers: string[] = [];
  if (model.capabilities?.includes("vision")) markers.push("▧");
  if (model.capabilities?.includes("reasoning")) markers.push("✦");
  if (model.capabilities?.includes("tools")) markers.push("⌘");
  ...
  <label class="model-select-wrap">
    ...
    <select class="model-select" :value="selected ? app.modelKey(selected) : ''" @change="(event) => { const value = (event.target as HTMLSelectElement).value; const model = app.models.find((item) => app.modelKey(item) === value); if (model) emit('select', model); }">
      ...
      <optgroup v-for="group in groups" :key="group.id" :label="group.label">
        <option v-for="model in group.models" :key="app.modelKey(model)" :value="app.modelKey(model)">{{ model.name }}{{ capabilityMarkers(model) }} {{ model.loaded ? '· Loaded' : '· Not loaded' }}</option>
      </optgroup>
    </select>
